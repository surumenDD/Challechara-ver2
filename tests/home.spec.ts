...
test.describe("ホーム画面", () => {
  test.beforeEach(async ({ page }) => {
    ...
    // ページの読み込み待ち
    await expect(
      page.locator("h1,span").filter({ hasText: "マイブック" })
    ).toBeVisible();
  });
  ...
  test("初期状態の確認", async ({ page }) => {
    ...
    await expect(page.locator('button[title="グリッド表示"]')).toHaveClass(
      /primary/
    );
  });
  ...
  test("Grid/List切替→検索→新規作成→カードクリックで遷移", async ({ page }) => {
    ...
    await expect(page.locator('button[title="リスト表示"]')).toHaveClass(
      /primary/
    );
    ...
    // エディタページに遷移することを確認
    await expect(
      page.locator("h1").filter({ hasText: "三分割エディタ" })
    ).toBeVisible();
  ...
  test("並び替え機能", async ({ page }) => {
    ...
    // ボタンのテキストが変更されることを確認
    await expect(
      page.locator('button:has(text("タイトル A→Z"))').first()
    ).toBeVisible();
  });
  ...
  test("EmptyState表示", async ({ page }) => {
    ...
    await expect(
      page.locator("text=別のキーワードで検索してみてください")
    ).toBeVisible();
  });
});
