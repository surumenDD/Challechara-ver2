...
export class MockProvider implements ChatProvider {
  async send(
    ...
    if (content.includes("検索") || content.includes("探し")) {
      mockResponse +=
        "こちらの検索結果をご確認ください。より詳細な情報が必要でしたら、具体的なキーワードをお教えください。";
    } else if (content.includes("書き方") || content.includes("表現")) {
      mockResponse +=
        "この表現について、いくつかのバリエーションをご提案します。文脈に応じて適切なものをお選びください。";
    } else if (content.includes("資料") || content.includes("情報")) {
      mockResponse +=
        "提供された資料から関連する情報を抽出しました。詳細については該当箇所をご参照ください。";
    } else {
      mockResponse +=
        "ご質問ありがとうございます。AIアシスタントとして、最適な回答をご提供するよう努めます。";
    }
